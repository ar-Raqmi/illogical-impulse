...
hl.bind("SUPER + U", hl.dsp.window.tag({ tag = "nobar", action = "toggle" }), { description = "Toggle titlebar" })
...
hl.bind("SUPER + Escape", hl.dsp.exec_cmd('PID=$(hyprctl activewindow -j | jq -r .pid) && kill -9 "$PID" && pkill -9 -P "$PID"'), { description = "Window: Forcefully zap a window" })
...
--# Positioning mode
hl.bind("SUPER + ALT + Space", hl.dsp.exec_cmd(hyprScripts .. "/toggle_float.sh"), { description = "Window: Float/Tile" })
...
hl.bind("SUPER + W", hl.dsp.layout("togglesplit"), { description = "Window: Toggle Split"} )
